...
class _MchatPageState extends State<MchatPage> {
  ...
  void initState() {
    super.initState();
    _loadQuestions();
    // Also try to load last saved M-CHAT result so that
    // answers and score are visible when returning to this page.
  ...
  /// Load last saved M-CHAT answers and score for this child (if any)
  /// so that when user comes back to the page, their previous result
  /// and choices are visible without re-answering.
  Future<void> _loadLastSavedResult() async {
    try {
      ...
      // Silent fail – page will still work without history.
    }
  }

...
import { Visibility as VisibilityIcon, Person, Visibility } from '@mui/icons-material'
...
const VisualDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const moderateRisk = visualSessions.filter(
        (s: any) => s.risk_level === 'moderate'
      ).length
      const lowRisk = visualSessions.filter(
        (s: any) => s.risk_level === 'low'
      ).length

      setChildren(childrenWithVisual)
      setSessions(visualSessions)
      setStats({
        totalChildren: childrenWithVisual.length,
        totalSessions: visualSessions.length,
        highRisk,
        moderateRisk,
        lowRisk,
      })
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Visibility sx={{ fontSize: 40, color: '#059669' }} />
        <Typography variant="h4">
          {t('visual_checking')} {t('dashboard')}
        </Typography>
      </Box>
                ...
                <Card>
                  <CardActionArea onClick={() => navigate(`/children/${child.id}`)}>
                    <CardContent>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {childSessions.length} {t('visual_checking')} {t('sessions')}
                      </Typography>
                    </CardContent>
                  </CardActionArea>
                </Card>

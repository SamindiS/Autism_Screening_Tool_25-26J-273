...
import { Hearing, Person, Visibility } from '@mui/icons-material'
import { useNavigate } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import { childrenApi, sessionsApi } from '../../services/api'
import { format } from 'date-fns'

const AuditoryDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const auditorySessions = allSessions.filter(
        (s: any) => s.session_type === 'auditory'
      )

      const childIdsWithAuditory = new Set(
        auditorySessions.map((s: any) => s.child_id)
      )
      const childrenWithAuditory = allChildren.filter((c: any) =>
        childIdsWithAuditory.has(c.id)
      )

      const highRisk = auditorySessions.filter(
        (s: any) => s.risk_level === 'high'
      ).length
      const moderateRisk = auditorySessions.filter(
        (s: any) => s.risk_level === 'moderate'
      ).length
      const lowRisk = auditorySessions.filter(
        (s: any) => s.risk_level === 'low'
      ).length

      setChildren(childrenWithAuditory)
      setSessions(auditorySessions)
      setStats({
        totalChildren: childrenWithAuditory.length,
        totalSessions: auditorySessions.length,
        highRisk,
        moderateRisk,
        lowRisk,
      })
    } catch (error) {
      console.error('Error loading auditory data:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Hearing sx={{ fontSize: 40, color: '#0EA5E9' }} />
        <Typography variant="h4">
          {t('auditory_checking')} {t('dashboard')}
        </Typography>
      </Box>
                ...
                <Card>
                  <CardActionArea onClick={() => navigate(`/children/${child.id}`)}>
                    <CardContent>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {childSessions.length} {t('auditory_checking')} {t('sessions')}
                      </Typography>
                    </CardContent>
                  </CardActionArea>
                </Card>
...
export default AuditoryDashboard

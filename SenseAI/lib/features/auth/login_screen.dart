...
class _LoginScreenState extends State<LoginScreen>
  ...
  void _addListeners() {
    _nameFocus.addListener(() =>
        _fieldControllers['name']!.animateTo(_nameFocus.hasFocus ? 1.0 : 0.0));
    _hospitalFocus.addListener(() => _fieldControllers['hospital']!
        .animateTo(_hospitalFocus.hasFocus ? 1.0 : 0.0));
    _pinFocus.addListener(() =>
        _fieldControllers['pin']!.animateTo(_pinFocus.hasFocus ? 1.0 : 0.0));
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_isLogin) {
        ...
        debugPrint(
            '🔐 PIN value (first 2): ${pin.length > 0 ? pin.substring(0, pin.length > 2 ? 2 : pin.length) + '***' : 'empty'}');
  ...
  Widget _buildAdvancedTextField({
    ...
    final animation =
        _fieldAnimations[fieldKey] ?? const AlwaysStoppedAnimation(0.0);

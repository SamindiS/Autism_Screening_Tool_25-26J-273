"""
Prepare Training and Test Datasets for Age 3.5-5.5 Frog Jump Model

Strategy:
- Training Set: External questionnaire data (auxiliary) + Hospital game data
- Test Set: Hospital game data (separate children)
- Features: Game metrics (primary) + Questionnaire scores (auxiliary)
"""
...
import numpy as np
import json
from config import AGE_3_5_5_5_CONFIG, PREPARED_DATA_DIR
...
def prepare_auxiliary_questionnaire_data():
    ...
    print("[PREP] Preparing Auxiliary Questionnaire Data for Age 3.5-5.5...")
    print("="*60)
    
    try:
        dataset_path = AGE_3_5_5_5_CONFIG['external_datasets'][0]['path']
        print(f"\nLoading: {dataset_path.name}...")
        df = pd.read_csv(dataset_path)
        
        # Filter to age 3.5-5.5
        df_filtered = df[(df['Age'] >= 42) & (df['Age'] < 66)]
        print(f"   Filtered: {len(df_filtered)} samples (age 42-66 months)")
...
def prepare_game_data():
    ...
    # You need to load your actual Frog Jump game data here
...
def main():
    """Main function"""
    print("\n" + "="*60)
    print("PREPARING DATASETS FOR AGE 3.5-5.5 FROG JUMP MODEL")
    print("="*60)

...
class VideoAnalysisService {
  ...
  static const double partialResponseThreshold =
      0.3; // minimum confidence for partial
  ...
  TriggerAnalysis _analyzeTriggerResponse({
    required List<VideoFrame> videoFrames,
    ...
    final postTriggerFrames = videoFrames.where((frame) {
      return frame.timestamp >= triggerTime &&
          frame.timestamp <=
              triggerTime + 5.0; // Analyze 5 seconds after trigger
    }).toList();
  ...
  DetectionResult _detectEyeGazeChange(
      VideoFrame current, VideoFrame previous) {
    // Simulate eye tracking analysis
    final gazeDirectionChange =
        (current.eyeGazeDirection - previous.eyeGazeDirection).abs();
  ...
  DetectionResult _detectFacialMovement(
      VideoFrame current, VideoFrame previous) {
    // Simulate facial landmark analysis
    final facialChange =
        (current.facialLandmarks - previous.facialLandmarks).abs();
  ...
  RTNStatus _classifyResponse({
    ...
    if (reactionTime <= immediateResponseThreshold) {
      ...
      final hasSignificantBehavior = behaviors
          .any((b) => b.confidence >= (partialResponseThreshold * 100).toInt());
...
class AnalysisResult {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'Detected_Behaviors': detectedBehaviors
          .map((b) => {
                'type': b.type.name,

  ...
  Future<void> _validateVideo(String filePath) async {
    ...
    try {
      ...
      if (!await file.exists()) {
        setState(() {
          _isValidating = false;
          _validationResult = {'passed': false, 'messages': ['File not found.'], 'checks': {}};
        });
        return;
      }
        ...
        setState(() {
          ...
          if (behaviors != null && behaviors.isNotEmpty) {
            _detectedActions = behaviors
                .map((b) {
                  if (b is Map) {
                    return b['type']?.toString().replaceAll('_', ' ').toUpperCase() ?? '';
                  }
                  return b.toString();
                })
          ...
          // Get ML Prediction results (if available)
          final mlPrediction = result['ML_Prediction'] as Map?;
          if (mlPrediction != null) {
            _mlPrediction = mlPrediction['prediction']?.toString();
            _autismProbability = (mlPrediction['autism_probability'] as num?)?.toDouble();
            _typicalProbability = (mlPrediction['typical_probability'] as num?)?.toDouble();
            _mlConfidence = (mlPrediction['confidence'] as num?)?.toDouble();
          } else {
            _mlPrediction = null;

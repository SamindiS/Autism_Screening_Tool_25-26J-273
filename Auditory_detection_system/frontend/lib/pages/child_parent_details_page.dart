...
class _ChildParentDetailsPageState extends State<ChildParentDetailsPage>
  ...
  void _handleContinue() {
    // Only child name and age are required to go to Video Analysis page
    final childName = _childNameController.text.trim();
    final ageText = _ageController.text.trim();
    final age = int.tryParse(ageText);
    final canGoToVideoAnalysis =
        childName.isNotEmpty && age != null && age >= 1 && age <= 6;

    if (canGoToVideoAnalysis) {
      ...
      _formKey.currentState?.validate(); // show errors on invalid fields
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please enter child name and age (1–6) to continue to Video Analysis.',
          ),
          backgroundColor: Color(0xFF7132C1),

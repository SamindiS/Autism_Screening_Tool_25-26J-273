"""
Complete Training Script for Age 5.5-6.9 Color-Shape Model

Includes:
- Data loading (DCCS game + auxiliary questionnaire)
- Feature engineering
- Outlier detection
...
from config import AGE_5_5_6_9_CONFIG, MODELS_DIR, PREPARED_DATA_DIR, OUTPUT_DIR
...
def load_data():
    """Load DCCS game data and auxiliary questionnaire data"""
...
def preprocess_data(df):
    ...
    preprocessor = DataPreprocessor(random_state=AGE_5_5_6_9_CONFIG['random_state'])
    df = preprocessor.clean_data(df)
    
    # Feature engineering
    feature_engineer = FeatureEngineer(
        age_bins=AGE_5_5_6_9_CONFIG['age_bins'],
        random_state=AGE_5_5_6_9_CONFIG['random_state']
    )
    
    df = feature_engineer.engineer_color_shape_features(df)
...
def handle_outliers(df):
    ...
    detector = OutlierDetector(
        method=AGE_5_5_6_9_CONFIG['outlier_detection']['method'],
        iqr_factor=AGE_5_5_6_9_CONFIG['outlier_detection']['iqr_factor']
    )
    
    all_features = AGE_5_5_6_9_CONFIG['game_features'] + AGE_5_5_6_9_CONFIG['auxiliary_features']
    numerical_cols = [col for col in all_features if col in df.columns and col != 'group']
    
    if AGE_5_5_6_9_CONFIG['outlier_detection']['winsorize']:
        df = detector.winsorize(
            df,
            numerical_cols,
            limits=AGE_5_5_6_9_CONFIG['outlier_detection']['winsorize_limits']
        )
...
def augment_data(X_train, y_train):
    """Augment training data (aggressive for small dataset)"""
    print("\n[AUGMENT] Augmenting training data (aggressive)...")
    
    augmenter = DataAugmenter(random_state=AGE_5_5_6_9_CONFIG['random_state'])
    
    if AGE_5_5_6_9_CONFIG['augmentation']['enabled']:
        X_aug, y_aug = augmenter.augment(
            X_train,
            y_train,
            method=AGE_5_5_6_9_CONFIG['augmentation']['method'],
            target_samples=AGE_5_5_6_9_CONFIG['augmentation']['target_samples']
        )
...
def train_models(X_train, y_train, X_test, y_test):
    ...
    lr_model = LogisticRegression(**AGE_5_5_6_9_CONFIG['logistic_regression'])
    ...
    rf_model = RandomForestClassifier(**AGE_5_5_6_9_CONFIG['random_forest'])
...
def save_model(model, scaler, feature_cols, model_name):
    ...
    model_path = MODELS_DIR / f"{AGE_5_5_6_9_CONFIG['model_name']}.pkl"
    joblib.dump(model, model_path)
    print(f"   [OK] Model saved: {model_path}")
    
    scaler_path = MODELS_DIR / f"scaler_{AGE_5_5_6_9_CONFIG['model_name']}.pkl"
    joblib.dump(scaler, scaler_path)
    print(f"   [OK] Scaler saved: {scaler_path}")
    
    features_path = MODELS_DIR / f"features_{AGE_5_5_6_9_CONFIG['model_name']}.json"
    with open(features_path, 'w') as f:
        json.dump(feature_cols, f, indent=2)
    print(f"   [OK] Features saved: {features_path}")
    
    metadata = {
        "model_name": AGE_5_5_6_9_CONFIG['model_name'],
        "age_range": AGE_5_5_6_9_CONFIG['age_range'],
        "session_type": AGE_5_5_6_9_CONFIG['session_type'],
        "features": feature_cols,
        "training_date": datetime.now().isoformat(),
        "algorithm": model_name
    }
    
    metadata_path = MODELS_DIR / f"model_metadata_{AGE_5_5_6_9_CONFIG['model_name']}.json"
...
def main():
    ...
    print("\n" + "="*60)
    print("TRAINING AGE 5.5-6.9 COLOR-SHAPE MODEL")
    print("="*60)
    ...
    X_train, X_test, y_train, y_test = preprocessor.split_train_test(
        combined_df,
        target_col='group',
        test_size=AGE_5_5_6_9_CONFIG['test_size']
    )
    
    # Augment data (aggressive)
    ...
    # Save results
    results_path = OUTPUT_DIR / f"training_results_{AGE_5_5_6_9_CONFIG['model_name']}.json"
